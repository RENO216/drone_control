...
class deep_navigation:
    def __init__(self,
        ...
        self.steer_sub = rospy.Subscriber(
            "steer_pre", Float32, self.steer_update)
        self.coll_sub = rospy.Subscriber("coll_pre", Float32, self.coll_update)
    ...
    def move(self):
        ...
        if abs(self.loc_x - self.dest_x) <= 0.1 and abs(self.loc_y - self.dest_y) <= destination_error:
            vel_msg.linear.x = 0
            vel_msg.angular.z = 0
        ...
        # during flying
            ...
            if self.time_interval >= adjust_interval:
                ...
                dest_angle = np.arctan(
                    (self.dest_y - self.loc_y)/(self.dest_x - self.loc_x))
